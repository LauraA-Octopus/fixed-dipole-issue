...
def ground_truth_with_drawing(image_path, output_path, grid_size=(7, 7), spacing=10):
    ...
    for i in range(grid_size[0]):
        for j in range(grid_size[1]):
            x = center_x + (j - (grid_size[1] // 2)) * spacing
            y = (center_y + (i - (grid_size[0] // 2)) * spacing) - 2
...
output_path = "/home/wgq72938/Documents/Hinterer/fixed-dipole-issue/mortensen/image_with_dipoles.png"
dipole_centroids = ground_truth_with_drawing(image_path, output_path)
    ...
    for contour in contours:
        if cv2.contourArea(contour) >= min_area:
            ...
            if M["m00"] != 0:
                ...
                cv2.circle(image, (cx, cy), radius=3, color=(255, 0, 0), thickness=-1)
